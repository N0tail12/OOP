...
public class MyDate {
	...
	public MyDate(int day, int month, int year) {
		if ((day >= 1 && day <= 31) || (month >= 1 && month <= 12)
			...
			if (!(day >= 1 && day <= 31))
                JOptionPane.showMessageDialog(null,"Error : Invalid day");
			if (!(month >= 1 && month <= 12))
                JOptionPane.showMessageDialog(null,"Error : Invalid month");
			if (year < 0)
                JOptionPane.showMessageDialog(null,"Error : Invalid year");
		}
	}
	...
	public void print() {
		JOptionPane.showMessageDialog(null,"My Date : " + month + " " + day + " " + year);
	}

	public void accept() {
		String date = JOptionPane.showInputDialog(null, "Enter the date(mm dd yy): ");
		int[] Date = validDate(date);
		if (Date != null) {
			month = Date[0];
			day = Date[1];
	...
	private int[] validDate(String date) {
		...
		if (m == 0) {
			JOptionPane.showMessageDialog(null,"Syntax of Error ! (Month)");
			error = 1;
		}
		int d = checkValidDay(day);
		if (d == 0) {
			JOptionPane.showMessageDialog(null,"Syntax of Error !(Day) ");
			error = 1;
		}
		int y = 0;
		if (isNumeric(year)) {
			...
			if (y < 0) {
				JOptionPane.showMessageDialog(null,"Syntax of Error ! (Year)");
				error = 1;
			}
		} else {
			JOptionPane.showMessageDialog(null,"Syntax of Error ! (Year)");
